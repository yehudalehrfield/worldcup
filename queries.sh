...
echo "$($PSQL "SELECT name FROM games FULL JOIN teams ON winner_id=team_id WHERE year=2014 AND round='Eighth-Final' UNION SELECT name FROM games FULL JOIN teams ON opponent_id=team_id WHERE year=2014 AND round='Eighth-Final' ORDER BY name")"
